...
export default function AdminData() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [userRes, deviceRes, locationRes, borrowRes] = await Promise.all([
          ...
          fetch("/api/location/getall?includeDevices=true", { credentials: "include" }),
        ...
        setLocations(locationData.data || locationData);
        setBorrows(borrowData.data || borrowData);
      } catch (err) {
  ...
  const pieData = {
    ...
    datasets: [{
      ...
      backgroundColor: Object.keys(zipRoleCounts).map((_, i) => `hsl(${(i * 360) / Object.keys(zipRoleCounts).length}, 70%, 60%)`),
    }],
  };

  const locationDeviceCounts = locations.reduce((acc, loc) => {
    const label = loc.city || loc.location_nickname || "Unknown";
    acc[label] = (acc[label] || 0) + (loc.device?.length || 0);
    return acc;
  }, {});
  ...
  const lateReturns = borrows.filter((b) => b.borrow_status === "Late").reduce((acc, b) => {
